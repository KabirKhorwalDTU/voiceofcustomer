...
- Added `docs/DEPLOYMENT.md`, `.github/workflows/render-backend.yml`, `backend/Dockerfile`, `scripts/cloud_smoke.py`, `scripts/set_vercel_api_url.sh`, and `scripts/finalize_cloud_deploy.sh` so backend deployment finalization can be run and verified as soon as Render access exists.
